...
# 获取筛选出的feat行号
data_feat_start = pd.read_excel(str_feat_start, sheet_name= 'silk_health15')   # 读取数据
...
for i_feat in range(len(feature_start1)):
    ...
    for j in range(feature_start-3,feature_start-2):
        ...
        for m in range(roi_num):
            ...
            plt.plot(x, y_krr, label=roi_name[m]+"内核岭回归", linestyle=line_styles[m],linewidth=5)
            plt.scatter(x,y,label=roi_name[m],s=80)
        ...
        # plt.ylabel('特征改变量(百分比)',fontproperties="SimSun")
        # plt.xlabel('时间（月）',fontproperties="SimSun")
        plt.ylabel('特征改变量/%',size= 20)
        plt.xlabel('时间/月',size= 20)
        legend=plt.legend(loc='best', fontsize=17)
        legend.get_frame().set_facecolor('#FFFACD')
        plt.gcf().subplots_adjust(bottom=0.1, left=0.1)  # 防止底部显示不全
        ...
        if mode == 1:  # 运行模式，1为保存图片，0，不保存
            ...
            plt.savefig(save_str, dpi=300)  # 保存汇总图像
