...
feature_start = [252]     # 特征开始的行数0为第一行,对应excel表行特征图1：287,270,46,58,174,61,7,280,185,
...
for i_feat in range(1):
    ...
    for i in range(len(patient_datatime)):
        ...
        # legend= plt.legend(loc=4, prop = {'size':16})   # 4右下角，1右上角
        # legend.get_frame().set_facecolor('#FFFACD')
        plt.ylabel('特征改变量/%', size=20)
        plt.xlabel('剂量/Gy', size=20)
        legend = plt.legend(loc='best', fontsize=20)
        legend.get_frame().set_facecolor('#FFFACD')
        plt.gcf().subplots_adjust(bottom=0.11, left=0.09)  # 防止底部显示不全
