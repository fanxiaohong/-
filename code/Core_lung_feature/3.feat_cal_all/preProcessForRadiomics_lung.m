function [volToEval,maskBoundingBox3M,gridS] = preProcessForRadiomics_lung(scanNum,...
...
%% zValsV出错时，补充一行算，总共需要添加两行语句此处为第一处
if (maxs-mins+1) <= length(zValsV)
    zValsV = zValsV(mins:maxs);
else
    zValsV=[zValsV,zValsV(end)+zValsV(2)-zValsV(1)];
end
