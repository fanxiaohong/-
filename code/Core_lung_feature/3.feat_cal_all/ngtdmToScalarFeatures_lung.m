function featuresS = ngtdmToScalarFeatures_lung(s,p,numVoxels)
% function featuresS = ngtdmToScalarFeatures(s,p,numVoxels)
% 
% APA, 3/15/2017

% Coarseness
featuresScoarseness = 1/(sum(s .* p) + 1e-6);
...
featuresScontrast = 1/Ng/(Ng-1) * term1 * term2 / numVoxels;
...
featuresSbusyness = sum(p .* s) / denom;
...
featuresScomplexity = complxty / numVoxels;
...
featuresSstrength = strength / sum(s);
featuresS = [featuresScoarseness;featuresScontrast;featuresSbusyness;featuresScomplexity;featuresSstrength];  % ×é×°
