function ivhFeaturesS = getIvhParams_lung(structNum, scanSet, IVHBinWidth,...
...
%1-4 calculate stats
ivhFeaturesSmeanHist = calc_meanDose(scanBinsV, volsHistV);
ivhFeaturesSmaxHist =  calc_maxDose(scanBinsV, volsHistV);
ivhFeaturesSminHist =  calc_minDose(scanBinsV, volsHistV);
ivhFeaturesSI50 = calc_Dx(scanBinsV, volsHistV,50);
%ivhFeaturesS.IVHBinWidth = IVHBinWidth;
%5 ivhFeaturesS.slopeAtD50 = calc_Slope(scanBinsV, volsHistV, ivhFeaturesS.I50, 0);

ivhFeaturesSrangeHist = ivhFeaturesSmaxHist - ivhFeaturesSminHist;
absFlag = 1;
...
for i = 1:length(xForVxV)
    absImgVal = xForVxV(i)*ivhFeaturesSrangeHist/100 + ivhFeaturesSminHist;
%     ivhFeaturesS.(['Vx',num2str(xForVxV(i))]) = calc_Vx(scanBinsV, volsHistV, absImgVal);
    eval(['ivhFeaturesSVx',num2str(xForVxV(i)),' = calc_Vx(scanBinsV, volsHistV, absImgVal);']);
end
for i = 1:length(xAbsForVxV)
...
% return
ivhFeaturesS = [ivhFeaturesSmeanHist;ivhFeaturesSmaxHist;ivhFeaturesSminHist;ivhFeaturesSI50;ivhFeaturesSrangeHist;...
    ivhFeaturesSVx0;ivhFeaturesSVx10;ivhFeaturesSVx20;ivhFeaturesSVx50;ivhFeaturesSVx80;ivhFeaturesSVx90;...
    ivhFeaturesSVx95]; % ×é×°
