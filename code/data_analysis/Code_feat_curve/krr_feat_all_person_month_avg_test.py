...
feature_start = 53      # 特征开始的行数0为第一行,对应excel表行特征
        ...
        for m in range(roi_num):
            ...
            for i in range(len(patient_datatime)):
                ...
                for k in range(patient_datatime[i]):  # 每个时间点求和并求平均值
                    y1 = trainData[roi_num * k + m]
                    y = y + y1
                yPost_pre = y / patient_datatime[i]
                X = np.array(datatime).reshape(-1, 1)
                yPost_pre_kr1.append(yPost_pre)
            yPost_pre_kr1 = np.array(yPost_pre_kr1)
            if l ==1: # 运行模式，1为start平移，0为original
                y_start = [yPost_pre_kr1[0]] * (len(datatime))   # 创建起点矩阵
                yPost_pre_kr = yPost_pre_kr1-np.array(y_start)
                plt.title('dose_response_people_start')  # 给图像加总标题
                plt.plot(X, yPost_pre_kr, label=roi_name[m])
            else:
                plt.title('dose_response_people')  # 给图像加总标题
                plt.plot(X, yPost_pre_kr1,label=roi_name[m])
        plt.ylabel('Δfeature(%)')
