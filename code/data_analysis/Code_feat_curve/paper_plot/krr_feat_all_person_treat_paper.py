...
str_data = 'E:\\roi_feat_dose\\result\\feature_all_person_treat_radiation.xls'
str_feat_start = 'E:\\roi_feat_dose\\result\\feat_filt_radiation.xls'   # 筛选出的特征结果存放位置
sheet_name = '2'  # 图片保存名称
pic_str = 'E:\\roi_feat_dose\\result\\treat\\' + sheet_name + '\\'
# pic_str = 'E:\\roi_feat_dose\\workflow\paper\\2\\'
# feature_start = [425]     # 特征开始的行数0为第一行,对应excel表行特征图1：287,270,46,58,174,61,7,280,185,
...
# 获取筛选出的feat行号
data_feat_start = pd.read_excel(str_feat_start, sheetname= sheet_name)   # 读取数据
data_feat = data_feat_start.values[:,:]

# 设置画布
train = pd.read_excel(str_data, sheetname=0)  # 读取数据
