function  plot_dose_response_feat(i,roi_num,roi_name,data_row,patient_name,data_time_1,data_str,data_time_2,...
    data_time_3,data_time_4,data_time_5,data_time_6,data_time_7,data_time_8,data_time_9,data_time_10,...
    num_dose_response,dose_x,save_str,line_style,print_select,data_time_11,data_time_12)
% 绘制各病人的特征变化剂量响应曲线图
...
%% 保存图像
figure_str = [save_str,char(txt_featname(i,1)),'.png'];  % figure保存的位置
...
%% 保存图像
figure_str = [save_str,char(txt_featname(i,1)),'_start.png'];  % figure保存的位置
set(gcf,'position',[100,100, 3000, 1500]); %设定figure的位置和大小 get current figure
set(gcf,'color','white'); %设定figure的背景颜色
if print_select == 1   % 判断是否打印图片
    print(gcf,'-dpng',figure_str)   %保存当前窗口的图像
