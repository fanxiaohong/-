function  plot_month_feat_pca(i,roi_num,roi_name,data_row,patient_name,data_time_1,data_str,data_time_2,data_time_3,...
...
%% 保存图像
figure_str = [save_str,num2str(i),'_LowDose.png'];  % figure保存的位置
...
%     title([roi_name(length(roi_name)-k+1),txt_featname(i,1)]);
    title(roi_name(length(roi_name)-k+1));
end 

%% 保存图像
figure_str = [save_str,num2str(i),'_HighDose.png'];  % figure保存的位置
set(gcf,'position',[100,100, 2000, 1000]); %设定figure的位置和大小 get current figure
set(gcf,'color','white'); %设定figure的背景颜色
if print_select == 1   % 判断是否打印图片
    print(gcf,'-dpng',figure_str)   %保存当前窗口的图像
