...
patient_datatime = [11,6,4,2] ;   % 放疗时间，各个病人诊断CT的时间点个数
save_mode = 0;  % 保存模式，如果为1，则保存
...
for i3 = 1:length(txt_featname)
    ...
    if (logistic23 >= 8) & (logistic34 >= 7)& (logistic12 >= 6) %如果符合条件则保存对应的特征数据和特征名
        for p = 1: length(sheet_name)   % 循环命名初始空数组
...
for i3 = 1:length(txt_featname_filt2)
    for i = 3:4   % 将1.5和4.2分别与\8\12\15对比统计
    ...
    if (logistic24 >= 8) & (logistic13 >= 8) %如果符合条件则保存对应的特征数据和特征名
        for p = 1: length(sheet_name)   % 循环命名初始空数组
...
for i3 = 1:length(txt_featname_filt3)
    for i = 4   % 将1.5和4.2分别与\8\12\15对比统计
    ...
    if (logistic14 >= 8) %如果符合条件则保存对应的特征数据和特征名
        for p = 1: length(sheet_name)   % 循环命名初始空数组
...
%         eval(['logistic',num2str(i-2),num2str(i),' = logistic1 ;']);  % 第一次筛选赋初始逻辑
    ...
    if (logistic34 >= 2)&(logistic23 >= 1)  %如果符合条件则保存对应的特征数据和特征名
        for p = 1: length(sheet_name)   % 循环命名初始空数组
...
for i3 = 1:length(txt_featname_filt5)
    ...
    if (logistic23 >= 4) & (logistic34 >= 4) %如果符合条件则保存对应的特征数据和特征名
        for p = 1: length(sheet_name)   % 循环命名初始空数组
