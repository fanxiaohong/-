...
%             aa = (abs(aa)>20) ;
            eval(['aa = (num_data_avg1_filt2(i3,j)-num_data_avg',num2str(i),'_filt2(i3,j));']);
            aa = (abs(aa)>5) ;
            logistic1 = logistic1 + aa;
        end
        eval(['logistic',num2str(i),' = logistic1;']);  % 第一次筛选赋初始逻辑
...
for i5 = 1:length(txt_featname_filt4)
    j = 6 ; % dose10-15
    if (abs(num_data_avg4_filt4(i5,j)-num_data_avg1_filt4(i5,j)) < 20) && (abs(num_data_avg3_filt4(i5,j)-num_data_avg1_filt4(i5,j)) < 10)
        for p = 1: length(sheet_name)   % 循环命名初始空数组
