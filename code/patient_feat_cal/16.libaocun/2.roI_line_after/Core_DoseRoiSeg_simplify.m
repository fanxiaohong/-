...
patient_name = '16.libaocun';
data_time = {'181225'} ;   %   181009
plan_time = {'180312'} ; % 计划时间
delta_x =  [-12.131] ;  % 单位mm，x方向偏移,变形后减去变形前，通过3Dslicer变形场可计算得到,程序xdose起点-254.426，与DCM文件符号相反
delta_y =  [-33.311] ;  % 单位mm，y方向偏移，27.9647，与DCM文件一致
delta_z =  [-31.942] ;  % 单位mm，y方向偏移，-6.1314，与DCM文件符号相反，顺序相反
dose_grid_space = 0.4 ;   % 剂量网格尺寸
dose_seg = 5 ;          % 剂量划分区间，dose bin
doseNum = 6;             % 用第几种剂量方案，周正元方案7，胡红军方案1
seg_linename = 'lung_seg' ; % 肺分割线名称
lung_mask = 1 ;          % 肺分割mask区域的值
%% plan超参数，读入mat文件，mat.bz2压缩过的不行
lung_start = 4 ;      % 肺部图像开始slicer
lung_end = 43 ;   % 肺部图像结束slicer
strName = 'lung';        % 肺名称
image_size = 512   ;   % 图片大小
