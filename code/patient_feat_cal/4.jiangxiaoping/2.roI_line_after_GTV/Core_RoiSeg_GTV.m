...
num_image = [24];         % 变形后GTV图片数量量
num_image_plan = [21];    % 计划的roi的dcm文件数量
roi_x =  [231.3311] ;  % 单位mm，x方向偏移,变形后减去变形前，通过3Dslicer变形场可计算得到,程序xdose起点-254.426，与DCM文件符号相反
roi_y =  [243.2972] ;  % 单位mm，y方向偏移，27.9647，与DCM文件一致
roi_z =  [-217.8359] ;  % 单位mm，y方向偏移，-6.1314，与DCM文件符号相反，顺序相反
roi_x_plan = 252.328;    % 计划roidcm坐标信息
roi_y_plan = 250 ;
roi_z_plan = -200.8 ;
roi_name = 'GTV'  ;      % roi的名称
