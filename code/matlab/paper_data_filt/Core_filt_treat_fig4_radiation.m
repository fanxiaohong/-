...
patient_datatime = [8,12,6,3,2] ;   % 各个病人诊断CT的时间点个数
...
for i3 = 1:length(txt_featname)
    for i = 3:5   % 将1.5和4.2分别与\8\12\15对比统计
        ...
        eval(['logistic2',num2str(i),' = 0 ;']);  % 第一次筛选赋初始逻辑
        logistic1 = 0; % 统计用
        logistic2 = 0; % 统计用
        logistic12 = 0 ; % 统计用
        ...
        eval(['logistic2',num2str(i),' = logistic2;']);  % 第一次筛选赋初始逻辑
    end
...
for i3 = 1:length(txt_featname_filt2)
    for i = 1:5   % 将1.5和4.2分别与\8\12\15对比统计
    ...
    if (logistic1 >= 4) & (logistic3 >=4)& (logistic4 >= 4) & (logistic5 >= 4)%如果符合条件则保存对应的特征数据和特征名
        for p = 1: length(sheet_name)   % 循环命名初始空数组
...
for i = 1:length(txt_featname_filt4)
    [x,y] = find(strcmp(txt_featname,txt_featname_filt4(i)));  % x,y分别是行向量和列向量，这里只用到x
