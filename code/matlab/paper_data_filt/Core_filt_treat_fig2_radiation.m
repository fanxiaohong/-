...
data_str = [result_str,'feature_all_person_treat_radiation.xls'];
...
for i2 = 1:length(txt_featname)
    ...
    if (logistic2 >= 8) & (logistic3 >= 8) & (logistic4 >= 8) %如果符合条件则保存对应的特征数据和特征名
        for p = 1: length(sheet_name)   % 循环命名初始空数组
...
for i4 = 1:length(txt_featname_filt3)
    for i = 3:4   % 将10次和20次、30次、2.5个月进行对比
        eval(['logistic',num2str(i),' = 0 ;']);  % 第一次筛选赋初始逻辑
        logistic1 = 0; % 统计用
        for j = 2 : 10  %9个剂量区间纳入考核
            eval(['aa = (num_data_avg2_filt3(i4,j)-num_data_avg',num2str(i),'_filt3(i4,j))/num_data_avg2_filt3(i4,j)*100;']);
            aa = (abs(aa)<50) ;
            logistic1 = logistic1 + aa;
        end
        eval(['logistic',num2str(i),' = logistic1;']);  % 第一次筛选赋初始逻辑
    end
    if  (logistic3 >= 7)& (logistic4 >= 7)  %如果符合条件则保存对应的特征数据和特征名
        for p = 1: length(sheet_name)   % 循环命名初始空数组
...
for i5 = 1:length(txt_featname_filt4)
    j = 6 ; % dose10-15
    if (num_data_avg4_filt4(i5,j)-num_data_avg1_filt4(i5,j)) * (num_data_avg2_filt4(i5,j)-num_data_avg1_filt4(i5,j))>0
        for p = 1: length(sheet_name)   % 循环命名初始空数组
...
if save_mode == 1 
    ...
    xlswrite(save_file,txt_featname_filt5,'2',clm_data);
    clm_data1 = ['B',num2str(2)];     % 把特征名称写入excel文件
    xlswrite(save_file,col_feat,'2',clm_data1);
end
