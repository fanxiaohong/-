...
for i, (train, test) in enumerate(cv.split(trainData, train_Label)):  # 10折模型训练并画图
    ...
    xg_eval_auc, xg_eval_acc, xg_eval_f1, xg_eval_recall, xg_eval_precision = result_print(train_Label[test],
                                                                                           test_transform_xg)
    ...
    precision, recall, thresholds = precision_recall_curve(train_Label[test], y_pred_xg)  # 绘制precision
    fpr = recall
    tpr = precision
...
ax.set(xlim=[-0.05, 1.05],title="Precision-recall curves")
