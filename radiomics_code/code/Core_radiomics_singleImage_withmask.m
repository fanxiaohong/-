...
image_type = 'mask' ;   % 图片的类型即文件夹名称covid或者noncovid
save_mode = 1 ;          % 保存模式，1保存，其余不保存
data_kind = 255 ;          % labelme为255，unet为1，计算图像为mask=1的地方
save_file = [dir_str,'result/',image_type,'_mask.xlsx'] ;
figure_mode = 1  ;       % 图像模式，1为画图验证，其他未不画图
...
for i =  1 : length(fileNames)
    ...
    mask_name = [mask_file_name,'/',image_order,'.tif'];   % 读取mask文件的名称
    image_name = [image_file_name,'/',image_order,'.tif'];   % 读取image文件的名称
...
%     dlmwrite(save_file,feat_all,'precision','%6.8f');
    ...
    xlswrite(save_file,feat_all,image_type,clm_data);
end
