...
str_data = '..\\result\\2class\\M1.xlsx'
str_name = '..\\result\\2class\\feature_all.csv'         # 纹理特征名称存储地址
...
train_Label_before = [0, 1]
num_noncovid = 60
num_covid = 60
train_Label1 = [0 for i in range(num_noncovid)]   # 制作标签397/576
train_Label2 = [1 for i in range(num_covid)]   # 制作标签349/484
...
for k in range(0,7):
    ...
    for j in range(len(start_image)):
        ...
        if bin_adapt_mode == 1:  # 判断是否是多窗宽，如果是则进行操作
            # 把15个窗宽数据堆叠程一列
            kmeans = []
            for l in range(num_covid + num_noncovid):
                trainData_kmeans = []
...
#####################################################################################
    ...
    #     list_acc = str(round(np.array(fold_acc_all).mean(),4))+'±'+str(round(np.array(fold_acc_all).std(),4))
    #     list_f1 = str(round(np.array(fold_f1_all).mean(),4))+'±'+str(round(np.array(fold_f1_all).std(),4))
    #     list_recall = str(round(np.array(fold_recall_all).mean(),4))+'±'+str(round(np.array(fold_recall_all).std(),4))
    #     list_precision = str(round(np.array(fold_precision_all).mean(),4))+'±'+str(round(np.array(fold_precision_all).std(),4))
    ...
    #     list_all = list(zip(*list_all))  # 转置列表
    #     print(list_all)
    # list_all1.append(list_all)
    # print(list_all1)
